...
import math
import time
from Lib.database_worker import connect_db, create_tables, insert_data, fetch_minutiae_data, insert_fingerprint, get_next_fingerprint_id
...
from Fingerprint_Extractor.ExtractionONE import center_fingerprint_roi
...
from Fingerprint_Extractor.Triplet_Extraction import process_extraction2, MinutiaPointHD
from Lib.global_matching import match_minutiae, euclidean_distance, angular_difference, MinutiaPointGM, calculate_minutiaesGM
import os
...
def calculate_similarity(minutiae_data_1, minutiae_data_2):
    # Placeholder for similarity calculation logic
    similarity_score = 0
    # Your logic here to calculate the similarity score
    return similarity_score
"""
def process_matching_data(db_path):
    conn = connect_db(db_path)
    candidate_rows = fetch_minutiae_data(conn)
     # Assuming probe_fingerprint_data is structured similarly to your database rows
    probe_minutiae_points = json.loads(probe_fingerprint_data['MinutiaePoints'])
    probe_distances = json.loads(probe_fingerprint_data['Distance'])
    probe_angles = json.loads(probe_fingerprint_data['Angles'])
    for row in candidate_rows:
        grid_id, minutiae_points_json, distances_json, angles_json = row
        candidate_minutiae_points = json.loads(minutiae_points_json)
        candidate_distances = json.loads(distances_json)
        candidate_angles = json.loads(angles_json)

        # Now, calculate similarity scores
        similarity_score = calculate_similarity(probe_minutiae_points, candidate_minutiae_points)
        # Here, you could track similarity scores, e.g., by adding them to a list or dictionary
