...
def process_fingerprint(image_path):
    ...
    #print(f"skeleton_img shape: {skeleton_img.shape}")
    ...
    #cv.imwrite(f'{output_dir}/singularity.jpg', result)
    ...
    if center:
        ...
        #cv.imwrite(f'{output_dir}/centered_skeleton.jpg', centered_img)
        #print(f"centered singularity shape: {centered_img.shape}")
    else:
        print("No singularities detected.")
        
    #singularities = merge_close_singularities(singularities)
    #print("Detected Singularities:")
    #for singularity in singularities:
        #print(f"Coordinates: ({singularity[0]}, {singularity[1]})") #just deleted "Type: {singularity[2]}" argument, add back if needed
