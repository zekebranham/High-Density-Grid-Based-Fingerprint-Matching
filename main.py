...
from Cascade2Fingerprint_Processor import process_cascade2, MinutiaPointHD
from global_matching import match_minutiae, euclidean_distance, angular_difference, MinutiaPointGM, calculate_minutiaesGM
...
#image path to process
    image_path1 = r"C:\Users\jonat\Documents\miniconda\High-Density-Grid-Based-Fingerprint-Matching\database\002\R\002_R3_3.bmp"
    image_path2 = r"C:\Users\jonat\Documents\miniconda\High-Density-Grid-Based-Fingerprint-Matching\database\002\R\002_R3_3.bmp"
    ...
    if centered_img is None:
        ...
        # Assuming raw_term_positions and raw_bif_positions include orientation data
        term_positions = [MinutiaPointHD(x, y) for x, y in raw_term_positions]
        bif_positions = [MinutiaPointHD(x, y) for x, y in raw_bif_positions]
...
# After obtaining minutiae positions, visualize them
        visualize_minutiae(img, term_positions, bif_positions, output_dir)


    """GLOBAL MATCHING STARTS HERE"""
    # Process the first fingerprint
    img1, mask1 = process_fingerprint(image_path1, output_dir)
    centered_img1 = center_fingerprint_roi(img1, mask1)
    minutiae_img1, term_positions1, bif_positions1 = calculate_minutiaesGM(centered_img1, mask1)

    img2, mask2 = process_fingerprint(image_path2, output_dir)
    centered_img2 = center_fingerprint_roi(img2, mask2)
    minutiae_img2, term_positions2, bif_positions2 = calculate_minutiaesGM(centered_img2, mask2)

    all_minutiae1 = term_positions1 + bif_positions1
    all_minutiae2 = term_positions2 + bif_positions2

# Perform global matching
    distance_threshold = 10  # This is a starting point; adjust based on empirical testing
    angle_threshold = np.radians(10)  # Convert 10 degrees to radians
    inter_matching_score = match_minutiae(all_minutiae1, all_minutiae2, distance_threshold, angle_threshold)

# Decision based on threshold
    if inter_matching_score >= 0.95:
        print("Fingerprints are considered a match.")
    else:
        print("Fingerprints do not match.")

# Visualize results
    visualize_minutiae(img1, term_positions1, bif_positions1, output_dir)
    visualize_minutiae(img2, term_positions2, bif_positions2, output_dir)

        
"""GLOBAL MATCHING ENDS HERE"""
